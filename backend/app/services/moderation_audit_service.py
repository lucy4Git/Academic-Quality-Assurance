"""Moderation Compliance Audit service.

Bridges the database and the ModerationComplianceAgent:

1. Load module and resolve institution_id.
2. Query files (READY, non-deleted) + their DocumentRecords -> build
   ModerationSnapshot (including ASSESSMENT_BRIEF / ASSESSMENT_PLAN files so
   the engine can run the date-sequencing check).
3. Call ModerationComplianceAgent.run(snapshot) -> ModerationAuditResult.
4. Persist AuditRun + AuditFinding rows (reusing shared tables, agent_type
   discriminator = MODERATION_COMPLIANCE).
...
This module mirrors ``assessment_audit_service.py`` (Stage 8) structurally —
the same load / snapshot / persist / error-handling pattern is reused.
"""
...
# Categories relevant to the moderation audit: the moderation documents
# themselves, plus the assessment documents needed for the date-sequencing
# check (item 12).
_RELEVANT_CATEGORIES: frozenset[FileCategory] = frozenset({
...
async def _build_snapshot(
    ...
    """Build a ModerationSnapshot from READY non-deleted files in the module folder.

    Only files in ``_RELEVANT_CATEGORIES`` are included — the moderation
    documents themselves plus the assessment plan/brief needed for the
    date-sequencing check. Files with a completed DocumentRecord get their
    extracted_text; all others get an empty string with has_extraction=False.
    """
    files_result = await db.execute(
        ...
        .where(
            ...
            File.category.in_(_RELEVANT_CATEGORIES),
        )
    ...
    moderation_files: list[ModerationFileInfo] = []
...
async def get_latest_run(
    ...
    """Return the most recent moderation compliance run for a module, or None."""
    result = await db.execute(
        select(AuditRun)
        .where(
            AuditRun.module_id == module_id,
            AuditRun.institution_id == institution_id,
            AuditRun.agent_type == AgentType.MODERATION_COMPLIANCE,
        )
...
async def list_runs(
    ...
    """Return paginated moderation audit runs for a module."""
    result = await db.execute(
        select(AuditRun)
        .where(
            AuditRun.module_id == module_id,
...
async def get_run_by_id(
    ...
    """Return a moderation AuditRun by ID (with findings) or raise NotFoundError."""
    result = await db.execute(
        select(AuditRun)
        .where(
            AuditRun.id == run_id,
    ...
    if run is None:
        raise NotFoundError("ModerationAuditRun", run_id)
    return run


async def run_moderation_audit(
    db: AsyncSession,
    run_id: uuid.UUID,
    module_id: uuid.UUID,
    institution_id: uuid.UUID,
) -> AuditRun | None:
    """Execute the moderation compliance audit.
