"""Outcome Alignment Audit service.

Bridges the database and the OutcomeAlignmentAgent:

1. Load module and resolve institution_id.
2. Query READY, non-deleted files in the module folder restricted to the
   categories relevant to outcome alignment (course outlines, study guides,
   learning outcomes, weekly plans, assessment plans/briefs/rubrics, exam
   papers, practical tasks, and evidence files) + their DocumentRecords ->
   build an ``OutcomeSnapshot`` (extracted text per file).
3. Call ``OutcomeAlignmentAgent.run(snapshot)`` -> ``OutcomeAuditResult``.
4. Persist AuditRun + AuditFinding rows (reusing shared tables, agent_type
   discriminator = OUTCOME_ALIGNMENT). No new database tables are created.
...
This module mirrors ``evidence_audit_service.py`` (Stage 11) structurally,
...
# Categories whose extracted text the engine inspects for outcome alignment.
...
async def _build_snapshot(
    ...
    """Build an OutcomeSnapshot from READY non-deleted files in the module
    folder, restricted to the categories relevant to outcome alignment.

    Files with a completed DocumentRecord get their extracted_text; all
    others get empty text / has_extraction=False.
    """
    ...
    outcome_files: list[OutcomeFileInfo] = []
    present_categories = set()
...
async def get_latest_run(
    ...
    """Return the most recent outcome alignment run for a module, or None."""
    result = await db.execute(
        select(AuditRun)
        .where(
            AuditRun.module_id == module_id,
            AuditRun.institution_id == institution_id,
            AuditRun.agent_type == AgentType.OUTCOME_ALIGNMENT,
        )
...
async def list_runs(
    ...
    """Return paginated outcome alignment runs for a module."""
    result = await db.execute(
        select(AuditRun)
        .where(
            AuditRun.module_id == module_id,
...
async def get_run_by_id(
    ...
    """Return an outcome alignment AuditRun by ID (with findings) or raise NotFoundError."""
    result = await db.execute(
        select(AuditRun)
        .where(
            AuditRun.id == run_id,
    ...
    if run is None:
        raise NotFoundError("OutcomeAlignmentAuditRun", run_id)
    return run


async def run_outcome_alignment_audit(
    db: AsyncSession,
    run_id: uuid.UUID,
    module_id: uuid.UUID,
    institution_id: uuid.UUID,
) -> AuditRun | None:
    """Execute the outcome alignment audit.
