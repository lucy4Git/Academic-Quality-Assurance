...
import { Archive, MessageSquare, Pencil, RotateCcw, Search, Trash2 } from "lucide-react";
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <div className="divide-y rounded-xl border bg-card">
        {visibleSessions.map((session) => <div key={session.id} className="flex items-center gap-3 p-4"><MessageSquare className="h-5 w-5 shrink-0 text-primary" /><div className="min-w-0 flex-1">{editingId === session.id ? <form onSubmit={(event) => submitRename(event, session.id)} className="flex gap-2"><label className="sr-only" htmlFor={`rename-${session.id}`}>Conversation title</label><input id={`rename-${session.id}`} autoFocus value={title} onChange={(event) => setTitle(event.target.value)} maxLength={255} className="min-w-0 flex-1 rounded border px-2 py-1 text-sm" /><button className="text-sm text-primary" type="submit">Save</button></form> : <Link href={`/workspace?session=${session.id}`} className="block rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"><span className="block truncate font-medium">{session.title || "Untitled conversation"}</span><span className="text-xs text-muted-foreground">{session.message_count} messages · {new Date(session.created_at).toLocaleString()}</span></Link>}</div><div className="flex gap-1"><button type="button" title="Rename" aria-label="Rename conversation" onClick={() => { setEditingId(session.id); setTitle(session.title || ""); }} className="rounded p-2 hover:bg-muted"><Pencil className="h-4 w-4" /></button><button type="button" title={archived ? "Restore" : "Archive"} aria-label={archived ? "Restore conversation" : "Archive conversation"} onClick={() => void run(() => conversationApi.archive(session.id))} className="rounded p-2 hover:bg-muted">{archived ? <RotateCcw className="h-4 w-4" /> : <Archive className="h-4 w-4" />}</button><button type="button" title="Delete" aria-label="Delete conversation" onClick={() => { if (window.confirm("Delete this conversation? This cannot be undone.")) void run(() => conversationApi.remove(session.id)); }} className="rounded p-2 text-destructive hover:bg-destructive/10"><Trash2 className="h-4 w-4" /></button></div></div>)}
      </div>
    </div>
  );
}
