"""Attendance Compliance Audit service.

Bridges the database and the AttendanceComplianceAgent:

1. Load module and resolve institution_id.
2. Query files (READY, non-deleted) + their DocumentRecords -> build
   AttendanceSnapshot (attendance register, tutorial, practical/lab, and LMS
   participation documents).
3. Call AttendanceComplianceAgent.run(snapshot) -> AttendanceAuditResult.
4. Persist AuditRun + AuditFinding rows (reusing shared tables, agent_type
   discriminator = ATTENDANCE_COMPLIANCE). No new database tables are created.
...
This module mirrors ``moderation_audit_service.py`` (Stage 9) structurally —
...
# Categories relevant to the attendance audit.
...
async def _build_snapshot(
    ...
    """Build an AttendanceSnapshot from READY non-deleted files in the module folder.

    Only files in ``_RELEVANT_CATEGORIES`` are included. Files with a
    completed DocumentRecord get their extracted_text; all others get an
    empty string with has_extraction=False.
    """
    ...
    attendance_files: list[AttendanceFileInfo] = []
...
async def get_latest_run(
    ...
    """Return the most recent attendance compliance run for a module, or None."""
    result = await db.execute(
        select(AuditRun)
        .where(
            AuditRun.module_id == module_id,
            AuditRun.institution_id == institution_id,
            AuditRun.agent_type == AgentType.ATTENDANCE_COMPLIANCE,
        )
...
async def list_runs(
    ...
    """Return paginated attendance audit runs for a module."""
    result = await db.execute(
        select(AuditRun)
        .where(
            AuditRun.module_id == module_id,
...
async def get_run_by_id(
    ...
    """Return an attendance AuditRun by ID (with findings) or raise NotFoundError."""
    result = await db.execute(
        select(AuditRun)
        .where(
            AuditRun.id == run_id,
    ...
    if run is None:
        raise NotFoundError("AttendanceAuditRun", run_id)
    return run
...
async def run_attendance_audit(
    ...
    """Execute the attendance compliance audit.
